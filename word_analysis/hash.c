...
/*向hash表中插入一组数据*/
void insert(struct HashTable *table, char *key, void *value)
{
	int index;
...
void del(struct HashTable *table, char *key)
{
	int index;
	struct HashNode *node;
	if(!table || !key)
...
int main(int argc, char **argv)
{
	...
	{
		insert(table, kws[i], kws[i]);
	}
	for(i = 0; i < sizeof(kws) / sizeof(char*); i ++)
	{
		del(table, kws[i]);
	}
	printf("%s %s\n", (char*)get(table, "hello"), (char*)get(table, "fuck"));
	insert(table, "hello", "bitch");
	printf("%s\n", (char*)get(table, "hello"));
